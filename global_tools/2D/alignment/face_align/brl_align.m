...
for i=1: length(subdir)
    ...
    for k=1: length(img_fns)
        ...
        %% while pose is so large and pts is wrong, we use centerloss alignment method. Otherwise, using bbox alignment
        if (f5pt(1,1)<f5pt(3,1) && f5pt(2,1)<f5pt(3,1)) || (f5pt(1,1)>f5pt(3,1) && f5pt(2,1)>f5pt(3,1))
            img_cropped=bbox_align_base(ffp_fn,img,imgSize);
        else
            if is_train
                img_size=imgSize(1);
...
function aligned_img=bbox_align_base(ffp_fn,img,imgSize)
